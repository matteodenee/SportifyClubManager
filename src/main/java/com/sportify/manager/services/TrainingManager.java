...
public class TrainingManager {
    ...
    public boolean createTraining(LocalDate date, LocalTime time, String location, String activity, int clubId) {
        ...
        Entrainement entrainement = new Entrainement(date, time, location, activity, clubId);
        return getDAO().create(entrainement);
    }

    public List<Entrainement> listUpcomingTrainings(int clubId, LocalDate fromDate) {
        if (clubId <= 0) {
