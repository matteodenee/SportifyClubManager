...
public class PostgresTeamDAO implements TeamDAO {
    ...
    public Team create(Team team) throws Exception {
        String sql = "INSERT INTO team (nom, categorie, clubid, coach_id, type_sport_id) VALUES (?, ?, ?, ?, ?) RETURNING id_team";
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            ...
            if (team.getCoachId() != null && !team.getCoachId().isBlank()) {
                pstmt.setString(4, team.getCoachId());
            } else {
    ...
    public Team findById(int id) throws Exception {
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            ...
            if (rs.next()) {
                ...
                t.setPlayers(findPlayersByTeamId(id));
                return t;
            }
        }
        return null;
    ...
    public List<Team> findAllByClubId(int clubId) throws Exception {
        ...
        String sql = "SELECT * FROM team WHERE clubid = ?";
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
    ...
    public void update(Team team) throws Exception {
        String sql = "UPDATE team SET nom=?, categorie=?, coach_id=?, type_sport_id=? WHERE id_team=?";
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            pstmt.setString(1, team.getNom());
            pstmt.setString(2, team.getCategorie());

            if (team.getCoachId() != null && !team.getCoachId().isBlank()) {
                pstmt.setString(3, team.getCoachId());
            } else {
    ...
    private Team mapResultSetToTeam(ResultSet rs) throws SQLException {
        ...
        int typeSportId = rs.getInt("type_sport_id");
        ...
        return new Team(
                ...
                rs.getInt("clubid"),
                (coachId == null || coachId.isBlank()) ? null : coachId,
